...
public class Lettore {
	...
	public void salvaDati( Color colori[], String avvio ) {
	...
	public Color[] leggiColori() {
		...
		if ( f.exists() ) {
			...
			try {
				...
				for ( int i = 0; i < coloriArr.length - 1; i++ ) {
	...
	public String leggiAvvio() {
		String ritorno = "Scelta gestori";
		...
		if ( f.exists() ) {
			...
			try {
				...
				String[] arr = testoIntero.split( "ª" );

				ritorno = arr[arr.length - 1];

			} catch ( Exception e ) {
				ritorno = "Scelta gestori";
			}
		}
